...
def test_missing_card(tmp_path: Path) -> None:
    dep = _clean_tree(tmp_path)
    # A capability with no card at all.
    _write_md(
        dep / "docs" / "capabilities" / "obs" / "nocard.md",
        {"id": "obs.nocard", "kind": "obs"},
    )
    findings = lint_content(dep)
    assert any(f.rule == "capability-card" and "obs/nocard" in f.location for f in errors(findings))
