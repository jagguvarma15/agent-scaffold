...
### Added
- **`agent-scaffold scaffold` — interactive REPL.** Persistent shell with slash commands (`/recipe`, `/language`, `/framework`, `/model`, `/effort`, `/plan`, `/cost`, `/go`, `/reset`, `/exit`), tab-completion for commands + recipe slugs, history at `~/.cache/agent-scaffold/repl_history`, Ctrl-D / Ctrl-L key bindings. Free-text input flows through the Haiku refinement interpreter; `/go` calls the same `pipeline.run_generation` that powers `agent-scaffold new`. Stays open until `/exit`, so multiple projects can be scaffolded in one session.
- **`prompt-toolkit>=3.0,<4`** added as an explicit dependency (previously transitive via questionary).
