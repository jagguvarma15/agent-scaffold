...
6. When generated code or config references an Anthropic model id, use exactly one of: `claude-sonnet-4-6` (the recommended default for a generated agent's runtime), `claude-sonnet-5`, `claude-haiku-4-5`, `claude-opus-4-8`, or `claude-fable-5`. These are complete ids. NEVER append a date suffix to them — ids like `claude-sonnet-4-6-20250514` do not exist and return 404 on the agent's first model call — and never invent any other id.
